<div class="flex flex-col gap-4">
    ...
    <div class="space-y-2">
        ...
        <div>{{__('To')}}: <x-badge> {{ $countEmails}} </x-badge></div>
    ...
    <div x-data="{ show: '{{ data_get($data, 'send_when', 'now') }}' }">
        ...
        <div class="flex flex-col gap-2 mt-2">
            ...
            <x-input.radio id="send_later" name="send_when" value="later" x-model="show">{{ __('Send Latter') }}</x-input.radio>
        </div>
        
        <div x-show="show == 'later'">
            <x-input.text id="send_at" class="block mt-1 w-full" type="date" name="send_at" :value="old('send_at')" autofocus />
