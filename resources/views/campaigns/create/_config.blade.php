<div class="grid grid-cols-2 gap-4">
    <div>
        ...
        <x-input.text id="name" class="block mt-1 w-full" name="name" :value="old('name', $data['name'])"
            autofocus />
    ...
    <div>
        ...
        <x-select id="email_list_id" name="email_list_id">
            <option value="" @if ( blank(old('email_list_id', $data['email_list_id'])) ) selected @endif>{{__('Please Select')}}</option>
    ...
    <div>
        ...
        <x-select id="template_id" name="template_id">
            <option value="" @if ( blank(old('template_id', $data['template_id'])) ) selected @endif>{{__('Please Select')}}</option>
    ...
    <div>
        <x-input.checkbox id="track_click" name="track_click" autofocus value="1" :isCheckWhen="old('track_click', $data['track_click'])" :label="__('Track Click')" />
        <x-input-error :messages="$errors->get('track_click')" class="mt-2" />
    </div>
    <div>
        <x-input.checkbox id="track_open" name="track_open" autofocus value="1" :isCheckWhen="old('track_open', $data['track_open'])" :label="__('Track Open')" />
